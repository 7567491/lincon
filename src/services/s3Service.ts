...
class S3Service {
  ...
  constructor() {
    ...
    this.config = {
      ...
      endpoint: import.meta.env.VITE_S3_ENDPOINT || 'https://us-east-1.linodeobjects.com'
    }
  ...
  // 获取存储桶中的对象列表
  async listObjects(bucket: string, prefix?: string, maxKeys = 1000): Promise<S3ListResponse> {
  ...
  // 生成预签名URL (用于直接上传/下载)
  async generatePresignedUrl(bucket: string, key: string, operation: 'get' | 'put', expiresIn = 3600): Promise<string> {
    if (!this.isConfigured()) {
      throw new Error('S3凭证未配置')
    }
