...
export class BillingService {
  ...
  calculateCrossMonthCost(
    ...
    while (currentStart < endTime) {
      ...
      const currentEnd = new Date(Math.min(monthEnd.getTime(), endTime.getTime()))
      
      const costData = this.calculateInstanceCost(instanceType, currentStart, currentEnd)
  ...
  private async addInstanceCostToDay(
    ...
    const targetMonthEnd = new Date(parseInt(targetYear), parseInt(targetMonth), 0, 23, 59, 59)
    
    // 确保时间范围在目标月份内
    const effectiveStart = new Date(Math.max(startTime.getTime(), targetDate.getTime()))
    const effectiveEnd = new Date(Math.min(endTime.getTime(), targetMonthEnd.getTime()))
    ...
    while (currentDay <= effectiveEnd) {
      const dayStart = new Date(Math.max(currentDay.getTime(), effectiveStart.getTime()))
      const dayEnd = new Date(Math.min(
        new Date(currentDay.getFullYear(), currentDay.getMonth(), currentDay.getDate(), 23, 59, 59).getTime(),
        effectiveEnd.getTime()
      ))
      
      if (dayStart < dayEnd) {
        const costData = this.calculateInstanceCost(instanceType, dayStart, dayEnd)
  ...
  private async processStorageLogs(
    ...
    const storageLogs = logs.filter(log => log.resourceType === 'object-storage')
   ...
   * 预测月底费用
   */
  async projectEndOfMonthCost(year: string, month: string): Promise<{
    currentSpend: number
