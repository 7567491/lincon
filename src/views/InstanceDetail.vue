  ...
  <div class="min-h-screen bg-gray-50">
    <header class="bg-white shadow">
      <div class="px-4 py-4 flex items-center">
        <button
          class="mr-4 text-blue-600 hover:text-blue-700"
          @click="$router.back()"
        >
          ← 返回
        </button>
          ...
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-lg font-semibold text-gray-900">
                {{ instanceStore.selectedInstance.label }}
              </h2>
              <p class="text-gray-600">
                ID: {{ instanceStore.selectedInstance.id }}
              </p>
            </div>
            <span
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="space-y-3">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium">{{
                instanceStore.selectedInstance.region
              }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">类型</span>
              <span class="font-medium">{{
                instanceStore.selectedInstance.type
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="space-y-3">
            <div class="flex justify-between">
              ...
              <span class="font-medium">{{
                instanceStore.selectedInstance.ipv4.join(", ")
              }}</span>
            </div>
            <div
              v-if="instanceStore.selectedInstance.ipv6"
              class="flex justify-between"
            >
              <span class="text-gray-600">IPv6地址</span>
              <span class="font-medium">{{
                instanceStore.selectedInstance.ipv6
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="space-y-3">
            <div class="flex justify-between">
              ...
              <span class="font-medium"
                >{{ instanceStore.selectedInstance.specs.vcpus }} vCPU</span
              >
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">内存</span>
              <span class="font-medium"
                >{{ instanceStore.selectedInstance.specs.memory }} MB</span
              >
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">存储</span>
              <span class="font-medium"
                >{{ instanceStore.selectedInstance.specs.disk }} GB</span
              >
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">流量</span>
              <span class="font-medium"
                >{{ instanceStore.selectedInstance.specs.transfer }} TB</span
              >
            </div>
          </div>
        </div>
      ...
      >
        <div class="bg-white rounded-lg p-6 max-w-sm mx-4">
          <h3 class="text-lg font-medium mb-4">
            确认{{ pendingAction.name }}实例
          </h3>
          <p class="text-gray-600 mb-6">
            您确定要{{ pendingAction.name }}实例 "{{
              instanceStore.selectedInstance?.label
            }}" 吗？
          </p>
          <div class="flex space-x-4">
            ...
            <BaseButton variant="secondary" @click="showConfirmDialog = false">
              取消
            </BaseButton>
          </div>
        </div>
...
const executeAction = async () => {
  ...
  try {
    await instanceStore.performAction(
      pendingAction.type,
      instanceStore.selectedInstance!.id,
    );
  } catch (error) {
