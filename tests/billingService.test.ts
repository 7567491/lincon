...
describe("BillingService", () => {
  ...
  describe("每日费用分析", () => {
    ...
    it("应该正确计算月度汇总", async () => {
      ...
      expect(summary.projectedMonthlyCost).toBeGreaterThanOrEqual(
        summary.monthToDateCost,
      );
    });
  });
  ...
  describe("费用预测", () => {
    it("应该基于历史数据预测月底费用", async () => {
      ...
      const projection = await billingService.projectEndOfMonthCost(
        "2025",
        "09",
      );
      ...
      expect(projection.projectedTotal).toBeGreaterThan(
