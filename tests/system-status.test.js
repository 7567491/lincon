...
describe('系统部署状态验证', () => {
  ...
  describe('服务端口监听状态', () => {
    ...
    it('应该在18080端口运行前端服务', async () => {
      try {
        const response = await axios.get('http://localhost:18080', { timeout: 5000 })
        expect(response.status).toBe(200)
        // 检查响应是否包含Vue应用标识
  ...
  describe('监控数据链路完整性', () => {
    it('应该能通过前端代理访问监控API', async () => {
      try {
        const response = await axios.get('http://localhost:18080/monitor-api/metrics', { timeout: 5000 })
        expect(response.status).toBe(200)
    ...
    it('应该能获取历史监控数据', async () => {
      try {
        const response = await axios.get('http://localhost:18080/monitor-api/history?minutes=15', { timeout: 5000 })
        expect(response.status).toBe(200)
    ...
    it('Python监控进程应该正在运行', () => {
      try {
        const output = execSync('ps aux | grep "monitor_daemon.py" | grep -v grep', { encoding: 'utf8' })
    ...
    it('监控日志文件应该存在且包含数据', () => {
      try {
        const output = execSync('ls -la /home/lincon/logs/*.json | wc -l', { encoding: 'utf8' })
        ...
        try {
          const logContent = execSync(`head -1 ${logFile}`, { encoding: 'utf8' })
          ...
          const latestLog = execSync('ls -t /home/lincon/logs/*.json | head -1', { encoding: 'utf8' }).trim()
          const logContent = execSync(`head -1 "${latestLog}"`, { encoding: 'utf8' })
          expect(logContent.trim()).not.toBe('')
        }
      } catch (error) {
...
describe('域名访问和反向代理验证', () => {
  ...
  it('域名应该能访问监控API', async () => {
    try {
      const response = await axios.get('http://con.linapp.fun/monitor-api/health', { timeout: 10000 })
      expect(response.status).toBe(200)
      expect(response.data).toHaveProperty('status', 'ok')
    } catch (error) {
